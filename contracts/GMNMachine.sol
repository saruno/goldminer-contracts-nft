...
contract GMNMachine is AccessControl, ERC721Enumerable {
    ...
    mapping(uint256 => MachineInfo) private machines;
    mapping(uint256 => string) private tokenURIs;
    mapping(bytes => bool) private boughtSignature;

    event Minted(address player, uint256 tokenId, MachineInfo info);
    event SetBaseUri(string baseUri);
    event ChangeName(uint256 tokenId, string name);
    event Sold(uint256 newTokenId, address player, MachineInfo characterInfo, uint256 price);
    ...
    function getMachine(uint256 tokenId) public view returns (MachineInfo memory) {
    ...
    function buy(MachineInfo calldata machine, uint256 price, uint256 expire, bytes memory signature) external payable returns(uint256) {
        require(verifySignature(machine, price, expire, signature), "Invalid Signature");
        ...
        uint256 newTokenId = mint(msg.sender, machine);
        emit Sold(newTokenId, msg.sender, machine, price);
        return newTokenId;
    } 

    //=== INTERNAL FUNCTION===
    function mint(address player, MachineInfo memory info) internal returns(uint256) {
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
        _mint(player, newTokenId);

        machines[newTokenId] = info;
    ...
    function verifySignature(MachineInfo memory info, uint256 price, uint256 expire, bytes memory signature) internal returns(bool) {
